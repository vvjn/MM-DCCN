...
from onmt.modules.CapsuleNetwork import CapsuleNet
from onmt.Utils import aeq
from onmt.modules import LayerNorm
from onmt.modules.ImgEmbeddings import ImgEmbeddings
import torch.nn.functional as F


class CapsuleMultiHeadedAttn(nn.Module):
    """
    ...
    """
    def __init__(self, head_count, model_dim, num_iterations, num_capsules, num_regions, dropout=0.1):

        assert model_dim % head_count == 0
        self.dim_per_head = model_dim // head_count
        self.model_dim = model_dim
        super(CapsuleMultiHeadedAttn, self).__init__()
        ...
        self.linear_attr = nn.Linear(model_dim, model_dim)
        self.final_linear = nn.Linear(model_dim, model_dim)
        self.layer_norm = LayerNorm(model_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, key, value, query, img, attr, img_mask=None, cap_mask=None, mask=None):
        """
        ...
        Args:
           ...
           img_mask: mask of image when using region features `[batch, num_region]`
        ...
        mix_img_attr, mix_cxt_attr = self.capsulenet_attr(attr_emb, context, img_mask=img_mask, text_mask=cap_mask)
        gate = F.sigmoid(self.linear_img(mix_cxt_img)+self.linear_attr(mix_cxt_attr))
        mix_context = gate*mix_cxt_img + (1-gate)*mix_cxt_attr
        output = self.final_linear(mix_context)
