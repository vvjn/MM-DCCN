...
onmt.opts.mmod_finetune_opts(parser)
...
def train_model(model, fields, optim, data_type, train_attr, valid_attr,
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        train_iter = make_dataset_iter(lazily_load_dataset("train"),
                                       fields, opt)
        # train_iter = make_dataset_iter(lazily_load_dataset("valid"),
        #                                fields, opt)
        train_stats = trainer.train(train_iter, epoch, report_func)
        print('Train perplexity: %g' % train_stats.ppl())
        print('Train accuracy: %g' % train_stats.accuracy())
        print('Train loss: %g' % train_stats.nloss())
        ...
        print('Validation loss: %g' % valid_stats.nloss())
...
#    valid_attr = valid_attr.astype(np.float32)
    ...
    # first_dataset = next(lazily_load_dataset("valid"))
    ...
    # Do training.
    train_model(model, fields, optim, data_type, train_attr, valid_attr,
                train_img_feats, valid_img_feats, train_img_mask, valid_img_mask, train_feat_indices,
                model_opt)
